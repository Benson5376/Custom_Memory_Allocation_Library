...
char* malloc(size_t ask_size)
{
    ...
    if (ask_size > 0) {
        ...
        Block* tmp_block_1 = BlockList;
        Block* tmp_block_t = NULL;
        size_t size_diff = 20000;

        for (; tmp_block_1 != NULL; tmp_block_1 = tmp_block_1->next) {
            if (tmp_block_1->free == 1 && tmp_block_1->size >= ask_size) {
                if (size_diff > (tmp_block_1->size - ask_size)) {
                    size_diff = (tmp_block_1->size - ask_size);
